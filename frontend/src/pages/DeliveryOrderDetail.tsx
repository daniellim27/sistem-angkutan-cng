...
const DeliveryOrderDetail: React.FC<DeliveryOrderDetailProps> = () => {
  ...
  const fetchSensorData = async () => {
    try {
      setIotLoading(true);
      setIotError(null);
      const response = await apiClient.get(`/web/delivery-orders/${id}/sensordata`);
      setSensorData(response.data.data || response.data);
      setLastUpdate(new Date());
    } catch (err) {
      setIotError('Failed to fetch IoT sensor data. Please check IoT connection.');
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow border-l-4 border-blue-500">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-semibold text-gray-900 mb-2">📡 IoT Sensor Data (Mockup)</h2>
                <p className="text-gray-600">Sample sensor data for delivery order #{deliveryOrder.do_number}</p>
              </div>
              <div className="text-right">
                ...
                <div className="text-xs text-gray-400">
                  {lastUpdate ? formatDate(lastUpdate.toISOString()) : 'Mock data loaded'}
                </div>
              </div>
            </div>
          </div>

          {/* IoT Data Display - Mock Data */}
          ...
          <div className="bg-gray-50 p-6 rounded-lg shadow">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">ℹ️ IoT System Information</h3>
