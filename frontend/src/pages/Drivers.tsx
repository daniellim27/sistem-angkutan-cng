...
const DriversPage = () => {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this CNG driver? This action cannot be undone.')) {
      try {
        ...
        alert('Failed to delete CNG driver.');
      }
    }
  };
  ...
  const getStatusDisplay = (driver: Driver) => {
    ...
    if (assignedVehicle) {
      return {
        text: `CNG Assigned to ${assignedVehicle}`,
    ...
    switch (driver.driverProfile.status) {
      ...
      default:
        return {
          text: `CNG ${driver.driverProfile.status}`,
          className: 'bg-gray-100 text-gray-800'
        };
    }
  };

  if (loading) return <div>Loading CNG drivers...</div>;
  if (error) return <div className="bg-red-100 text-red-700 p-4 rounded">{error}</div>;

  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">CNG Driver Management</h1>
      ...
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        <table className="min-w-full leading-normal">
          <thead>
            <tr>
              <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase">CNG Full Name</th>
              <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase">CNG Phone Number</th>
              <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase">CNG Status</th>
              <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-right text-xs font-semibold text-gray-600 uppercase">CNG Actions</th>
            </tr>
          </thead>
          <tbody>
            {drivers.map((driver) => {
              ...
              return (
                <tr key={driver.id}>
                  ...
                  <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm text-right">
                    <Link to={`/drivers/edit/${driver.id}`} className="text-indigo-600 hover:text-indigo-900 mr-4">Edit CNG Driver</Link>
                    <button onClick={() => handleDelete(driver.id)} className="text-red-600 hover:text-red-900">Delete CNG Driver</button>
                  </td>
                </tr>
              );
            })}
