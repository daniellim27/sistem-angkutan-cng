...
const isSPBGTransaction = (transaction: CashTransaction) => {
  // Check if this is an SPBG transaction by category_id = 56 (SPBG Deposit) or by SPBG fields
  return transaction.category_id === 56 || 
...
const CashCoordinatorPage = () => {
  ...
  useEffect(() => {
    const fetchAccounts = async () => {
      try {
      const response = await apiClient.get('/cash-coordinator/accounts');
  ...
  const fetchTransactions = useCallback(async () => {
    try {
      ...
      const response = await apiClient.get(`/cash-coordinator/transactions?${params}`);
      console.log('🔍 API Response:', response.data);
      console.log('📊 Transactions count:', response.data.data?.length || 0);
  ...
  const fetchCategories = useCallback(async () => {
    try {
      const response = await apiClient.get('/cash-coordinator/categories');
  ...
  // Debug: Monitor transactions state
  useEffect(() => {
  ...
  useEffect(() => {
    if (showModal && formData.category_id === '56') {
      fetchJisdorRate();
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSpecialCategory) {
      ...
      return; // Exit early
        ...
        if (formData.category_id === '56') { // SPBG category ID is 56
          if (formData.gas_volume_m3) {
    ...
    if (formData.category_id === '56' && currentJisdorRate) {
    ...
    try {
      ...
      if (editingTransaction) {
        await apiClient.put(`/cash-coordinator/transactions/${editingTransaction.id}`, submissionData, config);
      } else {
        await apiClient.post('/cash-coordinator/transactions', submissionData, config);
      }
  ...
  const handleEdit = (transaction: CashTransaction) => {
    ...
    setFormData({
      ...
      category_id: isSPBG ? '56' : (transaction.category_id?.toString() || ''),
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await apiClient.delete(`/cash-coordinator/transactions/${id}`);
      fetchTransactions();
    } catch (err) {
  ...
  if (loading) return <div className="text-center p-8">Loading cash coordinator transactions...</div>;

  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Cash Coordinator</h1>
        <button
          onClick={() => {
            resetForm();
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        {transactions.length === 0 && !loading && (
          <div className="text-center py-10 text-gray-500">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">Belum Ada Transaksi Coordinator</h3>
            <p className="text-gray-500 mb-4">Mulai dengan membuat transaksi kas coordinator pertama Anda.</p>
          </div>
        )}
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {!isSpecialCategory && formData.category_id !== '56' && (
                  <>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        Jumlah *
...
export default CashCoordinatorPage;