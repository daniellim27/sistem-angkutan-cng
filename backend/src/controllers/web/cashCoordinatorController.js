// backend/src/controllers/web/cashCoordinatorController.js
...
// Get all cash coordinator transactions with summary
exports.getAllCashCoordinatorTransactions = async (req, res, next) => {
  try {
    const { 
      page = 1, 
      limit = 20, 
    ...
    let whereClause = {
      ...
      // Only include coordinator transactions
      account: {
        [Op.like]: '%Coordinator%'
      }
    };
    ...
    console.error('Error in getAllCashCoordinatorTransactions:', err);
    next(err);
  }
};
...
exports.getAllTempoTransactions = async (req, res, next) => {
  try {
    ...
    if (account && account !== 'All' && account.includes('Coordinator')) {
      whereClause.account = account;
    }
...
// Create new cash coordinator transaction
exports.createCashCoordinatorTransaction = async (req, res, next) => {
  if (!req.body) {
  ...
  try {
    ...
    res.status(201).json({
      success: true,
      message: 'Cash coordinator transaction created successfully',
      data: {
    ...
    await transaction.rollback();
    console.error('Error in createCashCoordinatorTransaction:', err);
    next(err);
  }
};

// Get cash coordinator transaction by ID
exports.getCashCoordinatorTransactionById = async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    if (!cashTransaction) {
      return res.status(404).json({
        success: false,
        message: 'Cash coordinator transaction not found'
      });
    }
    ...
    console.error('Error in getCashCoordinatorTransactionById:', err);
    next(err);
  }
};

// Update cash coordinator transaction
exports.updateCashCoordinatorTransaction = async (req, res) => {
  const { id } = req.params;
  const {
    transaction_type,
    category_id,
  ...
  try {
    ...
    console.error('Error in updateCashCoordinatorTransaction:', error);
    return res.status(500).json({ message: 'Failed to update transaction', error: error.message });
  }
};

// Delete cash coordinator transaction
exports.deleteCashCoordinatorTransaction = async (req, res, next) => {
  const transaction = await db.sequelize.transaction();
  ...
  try {
    ...
    res.json({
      success: true,
      message: 'Cash coordinator transaction deleted successfully'
    });
  } catch (err) {
    await transaction.rollback();
    console.error('Error in deleteCashCoordinatorTransaction:', err);
    next(err);
  }
};
